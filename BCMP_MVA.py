...
class BCMP_MVA:
    ...
    def __init__(self, N, R, K, mu, type_list, p, m):
        self.N = N #網内のNumber of nodes(プログラム内部で指定する場合は1少なくしている)
...
if __name__ == '__main__':
    ...
    mu = np.full((R, N), 1)
    type_list = np.full(N, 1)
    m = np.full(N, 1)
    p = pd.read_csv('transition_probability_std_33_2_500.csv', header=None).values.tolist()
